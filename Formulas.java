...
public final class Formulas
{
	...
	public static final double calcHpRegen(Creature cha)
	{
		...
		double result = cha.calcStat(Stats.REGENERATE_HP_RATE, init, null, null) * hpRegenMultiplier + hpRegenBonus;
		if (cha instanceof Player p && p.isPhantom())
			result *= FakePlayerConfig.FAKE_HP_REGEN_MULTIPLIER;
		return result;
	}
	...
	public static final double calcMpRegen(Creature cha)
	{
		...
		double result = cha.calcStat(Stats.REGENERATE_MP_RATE, init, null, null) * mpRegenMultiplier + mpRegenBonus;
		if (cha instanceof Player p && p.isPhantom())
			result *= FakePlayerConfig.FAKE_MP_REGEN_MULTIPLIER;
		return result;
	}
	...
	public static final double calcCpRegen(Player player)
	{
		...
		double result = player.calcStat(Stats.REGENERATE_CP_RATE, init, null, null) * cpRegenMultiplier;
		if (player.isPhantom())
			result *= FakePlayerConfig.FAKE_CP_REGEN_MULTIPLIER;
		return result;
	}
	...
	public static double calcBlowDamage(Creature attacker, Creature target, L2Skill skill, byte shld, boolean ss)
	{
		...
		if (attacker instanceof Player playerAttacker && target instanceof Player playerTarget && !(target instanceof Monster)){
			/** Adventurer VS */
			if (playerAttacker.isInOlympiadMode() && playerTarget.isInOlympiadMode())
			{
				damage = damage*OlyClassDamageManager.getDamageMultiplier(playerAttacker, playerTarget);		
			}
			else
			{
				if (playerAttacker.getClassId().getId() == 8 || playerAttacker.getClassId().getId() == 93)
				{
					if (playerTarget.getClassId().getId() == 2 || playerTarget.getClassId().getId() == 88)
	...
	public static final double calcPhysDam(Creature attacker, Creature target, L2Skill skill, byte shld, boolean crit, boolean ss)
	{
		...
		if (attacker instanceof Player playerAttacker2 && target instanceof Player playerTarget2 && !(target instanceof Monster)){
			/** Dueslista VS */
			if (playerAttacker2.isInOlympiadMode() && playerTarget2.isInOlympiadMode())
			{
				damage = damage*OlyClassDamageManager.getDamageMultiplier(playerAttacker2, playerTarget2);		
			}
			else
			{
				if (playerAttacker2.getClassId().getId() == 2 || playerAttacker2.getClassId().getId() == 88)
				{
					if (playerTarget2.getClassId().getId() == 3 || playerTarget2.getClassId().getId() == 89)
	...
	public static final double calcMagicDam(Creature attacker, Creature target, L2Skill skill, byte shld, boolean ss, boolean bss, boolean mcrit)
	{
		...
		if (attacker instanceof Player playerAttacker3 && target instanceof Player playerTarget3 && !(target instanceof Monster)){
			/** Archmage VS */
			if (playerAttacker3.isInOlympiadMode() && playerTarget3.isInOlympiadMode())
			{
				damage = damage*OlyClassDamageManager.getDamageMultiplier(playerAttacker3, playerTarget3);		
			}
			else
			{
	...
	public static final void calcCastBreak(Creature target, double dmg)
	{
		...
		if (target instanceof Player player && player.getFusionSkill() != null)
		{
			target.breakCast();
			return;
		}
